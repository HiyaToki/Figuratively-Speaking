...
def get_ground_truth(filepath):
    raw_ground_truth = load_json(os.path.abspath(os.path.join(filepath, "ground-truth.json")))
    ground_truth = dict()
    
    for raw_element in raw_ground_truth["ground-truth"]:
...
def load_pan06(filepath):   
    ground_truth = get_ground_truth(filepath)
    dataset = []
    
    for candidate in os.listdir(filepath):
        candidate_path = os.path.join(filepath, candidate)
        
        # skip info files
        if candidate in ["ground-truth.json", "problem-info.json", "meta-file.json"]:
            continue
...
def document_predictions(dataset, sbert_model, joint_model, joint_tokenizer, label2id, id2label, thresholds):
    ...
    for i in tqdm(range(len(dataset)), desc = "PAN06 PROGRESS", ncols = 100):
        text = dataset[i]["text"]
...
dataset = load_pan06(base_dir + "/data/pan06/houvardas06-authorship-attribution-test-dataset")
...
dataset = add_tfidf_features(dataset)

# add char n-grams
dataset = add_tfidf_features(dataset, "char")

save_json(dataset, base_dir + "/data/pan06/pan06-authorship-attribution-test-dataset.json")